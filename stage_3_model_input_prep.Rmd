...
Load env.
```{r}
env=read_rds("./res_5_env/bottomT_df_h3_res_5_2.rds") %>% 
  left_join(read_rds("./res_5_env/sal_df_h3_2_res_5.rds"),
            by=c("h3_id","year")) %>% 
  left_join(read_rds("./res_5_env/ml_df_h3_res_5.rds"),
            by=c("h3_id","year")) %>% 
...
# Calculate bounding box of your points
bbox <- l_raja_gbif %>%
  summarise(
    xmin = min(lon)-3,
    xmax = max(lon)+3,
...
ggsave(plot = gbif_map_time,filename = "gbif_map_time.png",device = "jpeg",width = 20,height = 20,units = "cm")
```
